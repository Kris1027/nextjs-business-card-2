...
export type Shooting = { x: number; y: number; vx: number; vy: number; life: number }
...
export function createNebulae(w: number, h: number, hue: number, isFirefox: boolean): Nebula[] {
  if (isFirefox) return []
...
export function updateNebulae(nebulae: Nebula[], dt: number, w: number, h: number): void {
